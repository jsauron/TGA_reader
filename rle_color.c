...
/*   Updated: 2019/03/16 16:52:39 by jsauron          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			loadnew(int bpp, unsigned char *new, unsigned char *file, int mode)
{
	...
	while (loop <= repet)
	{
		...
		else if (bpp == 16 || bpp == 15)
		{
			...
			new[index] = ((file[2] & 0x7C) >> 2) * 255 / 31;
			new[index + 1] = (((file[2] & 0x3) << 3) * 255 / 31);
			new[index + 1] += (((file[1] & 0xE0) >> 5) * 255 / 31);
			new[index + 2] = (file[1] & 0x1F) * 255 / 31;
		}
		else if (bpp == 24)
		{
			new[index] = file[3];
